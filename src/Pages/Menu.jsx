        ...
        <div className="mt-15 mx-auto grid grid-cols-1 justify-center gap-x-10 gap-y-20 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 xl:gap-x-12 ">
          {Menuproducts.map((product) => (
            <div
              key={product.id}
              data-id={product.id}
              className={`menu-card group relative shadow-lg shadow-gray-400 rounded-b-lg transition-all duration-700 ease-out will-change-transform ${visibleCards[product.id] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} hover:-translate-y-1 hover:shadow-2xl hover:shadow-gray-500/20 cursor-pointer`}
            >
              <img
                alt={product.name}
                src={product.image}
